...
namespace OPM.OPMEnginee
{
    ...
    {
        private string dPId = @"XXXX/" + DateTime.Now.ToString("yyyy");
        public string DPId
        {
            get => dPId;
            set
        ...
        public int DPQuantity { get; set; } = 000;
        public int DPQuantity1 { get; set; } = 000;
        ...
        public string DPVNPTTechANSVContractNumber { get; set; } = "Số HĐ VNPT Tech";
        ...
        {
            string query = string.Format(@"SELECT SUM(DPQuantity1) FROM dbo.DP WHERE DPType = 0 AND POId = '{0}'", POId);
        ...
        public static bool DPVNPTTechANSVContractNb(string DPVNPTTechANSVContractNumber)
        {
            string query = string.Format("SELECT * FROM dbo.DP WHERE DPVNPTTechANSVContractNumber = N'{0}'", DPVNPTTechANSVContractNumber);
            DataTable table = OPMDBHandler.ExecuteQuery(query);
            return table.Rows.Count > 0;
        }

        public static bool DP_ID(string DPId, string POId)
        {
            string query = string.Format("SELECT * FROM dbo.DP WHERE DPId = '{0}' and POId = '{1}'", DPId, POId);
            DataTable table = OPMDBHandler.ExecuteQuery(query);
            return table.Rows.Count > 0;
        }
    }
