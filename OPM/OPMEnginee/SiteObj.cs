...
namespace OPM.OPMEnginee
{
    public class SiteObj
    {
        ...
        public static bool SiteANames(string SiteName)
        {
            string query = string.Format("SELECT SiteName FROM dbo.SITE WHERE SiteName = N'{0}'", SiteName);
            DataTable table = OPMDBHandler.ExecuteQuery(query);
            return table.Rows.Count > 0;
        }

        public static string VNPT_ID(string SiteName)
        {
            string query = string.Format(@"SELECT SiteId FROM dbo.Site WHERE SiteName = N'{0}'", SiteName);
