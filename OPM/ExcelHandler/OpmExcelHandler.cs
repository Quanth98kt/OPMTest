...
namespace OPM.ExcelHandler
{
    class OpmExcelHandler
    {
        ...
        public static string Temp27_PLReportForDelivery(string PLId)
        {
            ...
            object path = @"D:\OPMTest\Template\Mẫu 27. Biên bản bàn giao HH kho ANSV.xlsx";
            ...
            try
            {
                ...
                object filename = string.Format(@"D:\OPMTest\{0}\{1}\DP{2}\Mẫu 27. Biên bản bàn giao HH kho ANSV_{3}.xlsx", pl.ContractId.Trim().Replace('/', '-'), pl.POName.Replace('/', '-'), pl.DPId.Replace('/', '-'), pl.VNPTId);
        ...
        public static string Temp16_DPExportRequestForm_VNPTTech(string DPId)
        {
            ...
            object path = @"D:\OPMTest\Template\Mẫu 16. Phiếu yêu cầu xuất kho TECH.xlsx";
            if (!File.Exists(path.ToString()))
            {
                MessageBox.Show(string.Format(@"Không tìm thấy {0}", path.ToString()));
                return string.Format(@"Không tìm thấy {0}", path.ToString());
            ...
            try
            {
                ...
                Directory.CreateDirectory(folder);
                object filename = string.Format(@"D:\OPMTest\{0}\{1}\DP{2}\Mẫu 16. Phiếu yêu cầu xuất kho TECH_{2}.xlsx", dP.ContractId.Trim().Replace('/', '-'), dP.POName.Replace('/', '-'), dP.DPId.Replace('/', '-'));
                xlWorkbook.SaveAs(filename, Type.Missing, Type.Missing,
                Type.Missing, Type.Missing, Type.Missing, XlSaveAsAccessMode.xlExclusive,
                Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing);
                MessageBox.Show(string.Format("Đã tạo file {0}", filename.ToString()));
        ...
        {
            DPObj dP = new DPObj(DPId);
            object path = @"D:\OPMTest\Template\Mẫu 14. DP do ANSV tạo.xlsx";
            if (!File.Exists(path.ToString()))
            {
                MessageBox.Show(string.Format(@"Không tìm thấy {0}", path.ToString()));
                return string.Format(@"Không tìm thấy {0}", path.ToString());
            ...
            try
            {
                ...
                string folder = string.Format(@"D:\OPMTest\{0}\{1}\DP{2}", dP.ContractId.Trim().Replace('/', '-'), dP.POName.Replace('/', '-'), dP.DPId).Replace('/', '-');
                Directory.CreateDirectory(folder);
                object filename = string.Format(@"D:\OPMTest\{0}\{1}\DP{2}\Mẫu 14. DP do ANSV tạo_{2}.xlsx", dP.ContractId.Trim().Replace('/', '-'), dP.POName.Replace('/', '-'), dP.DPId.Replace('/', '-'));
                xlWorkbook.SaveAs(filename, Type.Missing, Type.Missing,
                Type.Missing, Type.Missing, Type.Missing, XlSaveAsAccessMode.xlExclusive,
                Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing);
                MessageBox.Show(string.Format("Đã tạo file {0}", filename.ToString()));
        ...
        {
            DPObj dP = new DPObj(DPId);
            object path = @"D:\OPMTest\Template\Mẫu 13. Phiếu yêu cầu xuất kho ANSV.xlsx";
            if (!File.Exists(path.ToString()))
            {
                MessageBox.Show(string.Format(@"Không tìm thấy {0}", path.ToString()));
                return string.Format(@"Không tìm thấy {0}", path.ToString());
            ...
            try
            {
                ...
                string folder = string.Format(@"D:\OPMTest\{0}\{1}\DP{2}", dP.ContractId.Trim().Replace('/', '-'), dP.POName.Replace('/', '-'), dP.DPId).Replace('/', '-');
                Directory.CreateDirectory(folder);
                object filename = string.Format(@"D:\OPMTest\{0}\{1}\DP{2}\Mẫu 13. Phiếu yêu cầu xuất kho ANSV_{2}.xlsx", dP.ContractId.Trim().Replace('/', '-'), dP.POName.Replace('/', '-'), dP.DPId.Replace('/', '-'));
                xlWorkbook.SaveAs(filename, Type.Missing, Type.Missing,
                Type.Missing, Type.Missing, Type.Missing, XlSaveAsAccessMode.xlExclusive,
                Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing);
                MessageBox.Show(string.Format("Đã tạo file {0}", filename.ToString()));
        ...
        {
            DPObj dP = new DPObj(DPId);
            object path = @"D:\OPMTest\Template\Mẫu 12. DP do OPM tạo.xlsx";
            if (!File.Exists(path.ToString()))
            {
                MessageBox.Show(string.Format(@"Không tìm thấy {0}", path.ToString()));
                return string.Format(@"Không tìm thấy {0}", path.ToString());
            ...
            try
            {
                ...
                string folder = string.Format(@"D:\OPMTest\{0}\{1}\DP{2}", dP.ContractId.Trim().Replace('/', '-'), dP.POName.Replace('/', '-'), dP.DPId).Replace('/', '-');
                Directory.CreateDirectory(folder);
                object filename = string.Format(@"D:\OPMTest\{0}\{1}\DP{2}\Mẫu 12. DP do OPM tạo_{2}.xlsx", dP.ContractId.Trim().Replace('/', '-'), dP.POName.Replace('/', '-'), dP.DPId.Replace('/', '-'));
                xlWorkbook.SaveAs(filename, Type.Missing, Type.Missing,
                Type.Missing, Type.Missing, Type.Missing, XlSaveAsAccessMode.xlExclusive,
                Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing);
                MessageBox.Show(string.Format("Đã tạo file {0}", filename.ToString()));
        ...
        public static string Temp7_PODistributionTable(string poid)
        {
            ...
            object path = @"D:\OPMTest\Template\Mẫu 7. Bảng phần bổ giá trị tạm ứng đơn hàng.xlsx";
            if (!File.Exists(path.ToString()))
            {
                MessageBox.Show(string.Format(@"Không tìm thấy {0}", path.ToString()));
                return string.Format(@"Không tìm thấy {0}", path.ToString());
            ...
            try
            {
                ...
                Directory.CreateDirectory(folder);
                object filename = string.Format(@"D:\OPMTest\{0}\{1}\Mẫu 7. Bảng phần bổ giá trị tạm ứng đơn hàng_{2}.xlsx", po.ContractId.Trim().Replace('/', '-'), po.POName.Replace('/', '-'), po.POId.Replace('/', '-'));
                xlWorkbook.SaveAs(filename, Type.Missing, Type.Missing,
                Type.Missing, Type.Missing, Type.Missing, XlSaveAsAccessMode.xlExclusive,
                Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing);
                MessageBox.Show(string.Format("Đã tạo file {0}", filename.ToString()));
        ...
        public static System.Data.DataTable DataTableNTKTDetailsFromExcelFile(string nameOfExcelFile, int indexWorksheet, int indexHeaderLine, int indexStartColumn)
        {
            System.Data.DataTable dataTable = new System.Data.DataTable();
            Microsoft.Office.Interop.Excel.Application excel = null;
            Microsoft.Office.Interop.Excel.Workbook workbook = null;
            ...
            try
            {
                ...
                // Worksheet
                sheet = (Microsoft.Office.Interop.Excel.Worksheet)workbook.Worksheets.Item[indexWorksheet];
                range = sheet.get_Range("A1:A100", System.Type.Missing).CurrentRegion;
                //var a = range.Range["A1:A100"];
                ...
                for (int i = indexHeaderLine + 1; i <= countOfRows; i++)
                {
                    if (!string.IsNullOrEmpty(Convert.ToString((range.Cells[i, 1] as Microsoft.Office.Interop.Excel.Range).Value2)))
                    {
                        DataRow dr = dataTable.NewRow();
                        for (int j = indexStartColumn; j <= countOfColumns; j++)
                        {
                            if (!string.IsNullOrEmpty(Convert.ToString((range.Cells[i, 1] as Microsoft.Office.Interop.Excel.Range).Value2)))
                            {
                                dr[j - indexStartColumn] = Convert.ToString((range.Cells[i, j] as Microsoft.Office.Interop.Excel.Range).Value);
